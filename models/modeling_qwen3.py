...
"""Modified from transformers.models.modeling_qwen3"""
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # CSC-Adapter注入（Attention之后，MLP之前）
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        """
        phonetic_features (`torch.FloatTensor` of shape `(batch_size, sequence_length, 6)`, *optional*):
            拼音特征输入，用于中文拼写纠错。每个token对应6维拼音特征向量。
        glyph_features (`torch.FloatTensor` of shape `(batch_size, sequence_length, 32, 32)`, *optional*):
            字形特征输入，用于中文拼写纠错。每个token对应32x32的汉字图像。
        """
        ...
        if attention_mask is not None:
            if attention_mask.dim() == 2:
                padding_mask = attention_mask  # 已经是 2-D
            elif attention_mask.dim() == 4:
                padding_mask = attention_mask[:, 0, 0, :].bool()  # [batch, seq_len]
            else:
